...
{
    [SerializeField]
    float speed, turnSpeed, deltaX, deltaZ;
    ...
    private void CheckInput()
    {
        ...
        if (h != 0 || v != 0)
        {
            ...
            if(!ValueInRange(targetPos.x, 0))   //player has reached a horizontal edges
            {
                ...
                Camera.main.transform.GetComponentInParent<CameraBehaviour>().SetApplyOffset(true, targetPos.x);
            }
            else
            {
                Camera.main.transform.GetComponentInParent<CameraBehaviour>().SetApplyOffset(false, targetPos.x);
            }
